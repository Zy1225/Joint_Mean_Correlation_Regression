...
rm(list=ls())
library(here)
i_am("Simulations/neg_binomial_simulation.R")
...
phi50 = rep(1,50)
A_half = (phi50*matrix(neg_binomial()$variance(MU), nrow = p , ncol = n))^(1/2)
...
for(b in 1:B){
  print(b)
  for (i in 1:n){
    data_Y[,b,i] = rnbinom(n = p, size = 1/phi50, mu = MU[,i])
  }
}

saveRDS(data_Y,here("Simulations","neg_binomial_data_Y.rds"))
...
for(p in p_vec){
  for(n in n_vec){
    ...
    reuse_path = here("Simulations","neg_binomial_data_Y.rds")
    
    #File path to save simulation result
    save_path = here('Simulations',paste0('neg_binomial_n',n,'_p',p,'_sim_result.rds'))
...
neg_binomial_list = vector('list',12)
count = 1
for(p in c(10,25,50)){
  for(n in c(50,100,200,400)){
    result_file_path = here('Simulations',paste0('neg_binomial_n',n,'_p',p,'_sim_result.rds'))
    sim_result = readRDS(result_file_path)
    neg_binomial_list[[count]] = sim_result
    count = count+1
  }
}

# MSE
dat_neg_binomial = expand.grid(c(50,100,200,400),c(10,25,50))
colnames(dat_neg_binomial) = c('n','p')
dat_neg_binomial$p = as.factor(dat_neg_binomial$p)

dat_neg_binomial$type = 'neg_binomial'
dat_neg_binomial$mse_beta = c(sapply(neg_binomial_list,FUN = function(x){
  B = dim(x$raw)[2];
  K = length(x$true_rho)
...
dat_neg_binomial$mse_rho = c(sapply(neg_binomial_list,FUN = function(x){
  B = dim(x$raw)[2];
  K = length(x$true_rho)
  d = length(x$true_beta) / x$p
  p = x$p
...
neg_binomial_mse_beta_plot = ggplot(data=dat_neg_binomial, aes(x=n, y=mse_beta,col = p, group=p)) +
  geom_line() + geom_point() + 
  ylab(expression(paste('MSE(',beta,')'))) + ggtitle('Negative Binomial')+ scale_y_continuous(labels =fmt_dcimals(3)) + theme_bw() +theme(plot.title = element_text(hjust = 0.5)) #+ guides(col = "none") 

neg_binomial_mse_rho_plot = ggplot(data=dat_neg_binomial, aes(x=n, y=mse_rho,col = p, group=p)) +
  geom_line() + geom_point() + 
  ylab(expression(paste('MSE(',rho,')'))) + ggtitle('Negative Binomial')+ scale_y_continuous(labels =fmt_dcimals(3)) + theme_bw() +theme(plot.title = element_text(hjust = 0.5)) #+ guides(col = "none") 

ggarrange(neg_binomial_mse_beta_plot,
          neg_binomial_mse_rho_plot,ncol = 1, common.legend = T, legend = 'bottom')


#Chi_sq coverage for hatbeta_S with S = {1,2,3,4,5} and hatrho
dat_neg_binomial$chisq_coverage_beta = sapply(neg_binomial_list,FUN = function(x){
  x$chisq_beta_est_coverage
})

dat_neg_binomial$chisq_coverage_rho = sapply(neg_binomial_list,FUN = function(x){
  x$chisq_rho_est_coverage
})

chisq_coverage_beta_table  = spread(dat_neg_binomial %>% dplyr::select(n,p, chisq_coverage_beta), key = p, value = chisq_coverage_beta)
chisq_coverage_rho_table = spread(dat_neg_binomial %>% dplyr::select(n,p, chisq_coverage_rho), key = p, value = chisq_coverage_rho)
...
plot_boxplot_coverage_beta(neg_binomial_list, title = 'Negative Binomial')

#Coverage for each of the correlation regression parameters
n_string = c('n=50','n=100','n=200','n=400')
neg_binomial_rho1_coverage = round(matrix(sapply(neg_binomial_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+1])}), nrow = 4, byrow = FALSE),3)
neg_binomial_rho2_coverage = round(matrix(sapply(neg_binomial_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+2])}), nrow = 4, byrow = FALSE),3)
neg_binomial_rho3_coverage = round(matrix(sapply(neg_binomial_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+3])}), nrow = 4, byrow = FALSE),3)
neg_binomial_rho4_coverage = round(matrix(sapply(neg_binomial_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+4])}), nrow = 4, byrow = FALSE),3)
neg_binomial_rho5_coverage = round(matrix(sapply(neg_binomial_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+5])}), nrow = 4, byrow = FALSE),3)

rho_element_coverage = rbind(cbind(n_string,neg_binomial_rho1_coverage),
                             cbind(n_string,neg_binomial_rho2_coverage),
                             cbind(n_string,neg_binomial_rho3_coverage),
                             cbind(n_string,neg_binomial_rho4_coverage),
                             cbind(n_string,neg_binomial_rho5_coverage))

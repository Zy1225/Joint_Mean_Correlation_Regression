...
rm(list=ls())
library(here)
i_am("Simulations/poisson_simulation.R")
...
set.seed(1)
for (i in 1:n){
  int_mat = overall.corr.mat(n.P = p, n.B = 0, n.O = 0, lambda.vec = c(MU[,i]), prop.vec = NULL, prop.list = NULL,
                             corr.vec = NULL, corr.mat = R0)
  final.corr.mat = int_mat
  data_Y[,,i] = t(gen.PoisBinOrd(B, n.P = p, n.B = 0, n.O = 0, lambda.vec = c(MU[,i]), prop.vec = NULL,
                                 prop.list = NULL, final.corr.mat))
}


saveRDS(data_Y,here("Simulations","poisson_data_Y.rds"))
...
for(p in p_vec){
  for(n in n_vec){
    ...
    reuse_path = here("Simulations","poisson_data_Y.rds")
    
    #File path to save simulation result
    save_path = here('Simulations',paste0('poisson_n',n,'_p',p,'_sim_result.rds'))
...
poisson_list = vector('list',12)
count = 1
for(p in c(10,25,50)){
  for(n in c(50,100,200,400)){
    result_file_path = here('Simulations',paste0('poisson_n',n,'_p',p,'_sim_result.rds'))
    sim_result = readRDS(result_file_path)
    poisson_list[[count]] = sim_result
    count = count+1
  }
}

# MSE
dat_poisson = expand.grid(c(50,100,200,400),c(10,25,50))
colnames(dat_poisson) = c('n','p')
dat_poisson$p = as.factor(dat_poisson$p)

dat_poisson$type = 'poisson'
dat_poisson$mse_beta = c(sapply(poisson_list,FUN = function(x){
  B = dim(x$raw)[2];
  K = length(x$true_rho)
...
dat_poisson$mse_rho = c(sapply(poisson_list,FUN = function(x){
  B = dim(x$raw)[2];
  K = length(x$true_rho)
  d = length(x$true_beta) / x$p
  p = x$p
...
poisson_mse_beta_plot = ggplot(data=dat_poisson, aes(x=n, y=mse_beta,col = p, group=p)) +
  geom_line() + geom_point() + 
  ylab(expression(paste('MSE(',beta,')'))) + ggtitle('Poisson')+ scale_y_continuous(labels =fmt_dcimals(3)) + theme_bw() +theme(plot.title = element_text(hjust = 0.5)) #+ guides(col = "none") 

poisson_mse_rho_plot = ggplot(data=dat_poisson, aes(x=n, y=mse_rho,col = p, group=p)) +
  geom_line() + geom_point() + 
  ylab(expression(paste('MSE(',rho,')'))) + ggtitle('Poisson')+ scale_y_continuous(labels =fmt_dcimals(3)) + theme_bw() +theme(plot.title = element_text(hjust = 0.5)) #+ guides(col = "none") 

ggarrange(poisson_mse_beta_plot,
          poisson_mse_rho_plot,ncol = 1, common.legend = T, legend = 'bottom')


#Chi_sq coverage for hatbeta_S with S = {1,2,3,4,5} and hatrho
dat_poisson$chisq_coverage_beta = sapply(poisson_list,FUN = function(x){
  x$chisq_beta_est_coverage
})

dat_poisson$chisq_coverage_rho = sapply(poisson_list,FUN = function(x){
  x$chisq_rho_est_coverage
})

chisq_coverage_beta_table  = spread(dat_poisson %>% dplyr::select(n,p, chisq_coverage_beta), key = p, value = chisq_coverage_beta)
chisq_coverage_rho_table = spread(dat_poisson %>% dplyr::select(n,p, chisq_coverage_rho), key = p, value = chisq_coverage_rho)
...
plot_boxplot_coverage_beta(poisson_list, title = 'Poisson')

#Coverage for each of the correlation regression parameters
n_string = c('n=50','n=100','n=200','n=400')
poisson_rho1_coverage = round(matrix(sapply(poisson_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+1])}), nrow = 4, byrow = FALSE),3)
poisson_rho2_coverage = round(matrix(sapply(poisson_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+2])}), nrow = 4, byrow = FALSE),3)
poisson_rho3_coverage = round(matrix(sapply(poisson_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+3])}), nrow = 4, byrow = FALSE),3)
poisson_rho4_coverage = round(matrix(sapply(poisson_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+4])}), nrow = 4, byrow = FALSE),3)
poisson_rho5_coverage = round(matrix(sapply(poisson_list,function(x){l=length(x$CI_coverage) - 5;return(x$CI_coverage[l+5])}), nrow = 4, byrow = FALSE),3)

rho_element_coverage = rbind(cbind(n_string,poisson_rho1_coverage),
                             cbind(n_string,poisson_rho2_coverage),
                             cbind(n_string,poisson_rho3_coverage),
                             cbind(n_string,poisson_rho4_coverage),
                             cbind(n_string,poisson_rho5_coverage))
